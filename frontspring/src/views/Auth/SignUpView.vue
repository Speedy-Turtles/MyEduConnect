<template>
  ...
  import service_classe from "@/service/ClasseService/class.js";
  import service_specilate from "@/service/specialite/spec.js";
  ...
  export default{
      ...
      created(){
          service_specilate.getSpecialte().then((response)=>{
               this.specialtes=response.data;
              
          })
          service_info.getALLRoles().then((res)=>{
            for(let i=0;i<(res.data).length;i++){
                this.items.push({"id":res.data[i].id,"RoleName":res.data[i].roleName});
            }
          })
      },
      validations:{
      ...
      data(){
          return{
              ...
              role:['ensignant','etudiant',"Technicien"],
              logo:require("../../assets/Logo_app/logo-no-background.png")
          }
      },
      methods:{
      /*  GetClassById(){
            service_classe.getClasseById(this.classe_id).then((res)=>{
                 this.classeCurrent.pop();
                 this.classeCurrent.unshift({"idclasse":res.data.idclasse});
            })
        },*/
        base64(){
        ...
        FixRoleSelected(){
            service_info.getRoleById(this.form.selectRole).then((res)=>{
                 this.roleSelected.pop();
                 this.roleSelected.unshift({"id":res.data.id,"RoleName":res.data.roleName});
                 this.CheckRole(res.data.roleName);
             })
         },
         /* saveImage(){
              ...
              if(this.form.selectRole=="etudiant"){
               ...
               this.loading=true;
                service_user.StoreUser({
                  ...
                  "sex":this.form.sex,
                  "cin":this.form.cin,
                  "classe":this.classe_id ? {"idclasse":this.classe_id}: null
                }).then((res)=>{
                      this.form.firstname="";
                      this.form.lastname="";
                      this.form.email="";
