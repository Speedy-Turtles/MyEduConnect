...
import service_info from "@/service/UserInfo/userinfo";
  export default{
      name:"forgotpassword",
      validations:{
        form:{
      ...
      methods:{
        ChangerPassword(){
            ...
            .then((res)=>{
                this.loading=false;
                this.$router.push({name:"signin",query:{content:res.data.data}});
            }).catch((error)=>{
                this.loading=false;
                this.snackbar=true;
      ...
      computed:{
        ...
        tokenerror(){
            ...
            !this.$v.form.code.numeric && error.push("Code Should be numeric");
